...
function normalizeBasePath(base, fallback) {
    ...
    if (!value) return '';
    // Board endpoints are same-origin directories, never queries or fragments.
    if (/[\\\u0000-\u0020\u007f?#]/.test(value) || value.startsWith('//')) return '';
    if (/^[a-z][a-z\d+.-]*:/i.test(value) && !/^https?:\/\//i.test(value)) return '';
    if (value.split('/').some(segment => segment && !isNoticePathSegmentSafe(segment))) return '';
    try {
        const url = new URL(value, window.location.href);
        if (!['http:', 'https:'].includes(url.protocol) || url.origin !== window.location.origin || url.username || url.password) return '';
    } catch {
        return '';
    }
...
function isNoticePathSegmentSafe(segment) {
    // Check encoded traversal/separators too; encode the original filename for the link.
    let decoded = segment;
    while (true) {
...
function sanitizeNoticeBody(html) {
    ...
    if (!purifier?.isSupported || typeof purifier.sanitize !== 'function') {
        throw new Error('Notice HTML sanitizer is unavailable');
    }
    // markdown-it emits table alignment as inline CSS. Keep only that exact
    // declaration as a safe enumerated HTML attribute; never allow style itself.
    const preserveTableAlignment = (node, data) => {
        if (data.attrName !== 'style' || !['th', 'td'].includes(node.localName)) return;
        const alignment = /^\s*text-align\s*:\s*(left|center|right)\s*;?\s*$/i.exec(data.attrValue);
        if (alignment) node.setAttribute('align', alignment[1].toLowerCase());
    };
    let fragment;
    purifier.addHook('uponSanitizeAttribute', preserveTableAlignment);
    try {
        fragment = purifier.sanitize(html, {
            ALLOWED_TAGS: [
                'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'br', 'hr', 'blockquote',
                'ul', 'ol', 'li', 'table', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td',
                'caption', 'pre', 'code', 'strong', 'em', 'b', 'i', 's', 'del',
                'a', 'img', 'sup', 'sub'
            ],
            ALLOWED_ATTR: ['href', 'src', 'alt', 'title', 'class', 'colspan', 'rowspan', 'scope', 'start', 'align'],
            ALLOW_DATA_ATTR: false,
            ALLOW_ARIA_ATTR: false,
            RETURN_DOM_FRAGMENT: true
        });
    } finally {
        purifier.removeHook('uponSanitizeAttribute', preserveTableAlignment);
    }
    // Narrow DOMPurify's URL policy for Notice: web images and web/mail/phone links.
    // Only remove attributes from the sanitized fragment; never reparse raw publisher HTML.
    for (const node of fragment.querySelectorAll('[href], [src]')) {
        for (const attribute of ['href', 'src']) {
            ...
            try {
                ...
                if (!protocols.includes(url.protocol) || url.username || url.password) node.removeAttribute(attribute);
            } catch {
    ...
    if (body) {
        ...
        try {
            ...
            if (notice.bodyFormat === 'html') {
                // Publisher disables raw Markdown HTML; this is an independent insertion boundary.
                const markdown = document.createElement('div');
                markdown.className = 'notice-markdown';
                markdown.appendChild(sanitizeNoticeBody(text));
...
function renderAttachments(attachments, attachmentsBase, assetVersion) {
    ...
    const listHtml = attachments
        .map((filename) => {
            ...
            if (filename.startsWith('/') || /^[a-z][a-z\d+.-]*:/i.test(filename)) return '';
            const segments = filename.split('/');
            if (segments.some(segment => !segment || !isNoticePathSegmentSafe(segment))) return '';
            ...
            return `
                <li class="attachment-item">
                    <a href="${escapeNoticeText(withVersion(normalizedBase + safeName))}" download class="attachment-link">
                        <span class="attachment-icon">📎</span>
                        ${escapeNoticeText(filename)}
                    </a>
                </li>
            `;
        })
