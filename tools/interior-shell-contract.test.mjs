...
const destinations = ['about', 'team', 'philosophy', 'perspective', 'portfolio', 'notice', 'contact'].map(section => `about.html#${section}`);
...
test('every interior page has the same reachable top navigation without an island or bottom sheet', async () => {
  for (const page of pages) {
    ...
    const links = [...header.matchAll(/<a href="([^"]+)"[^>]*>(About|Team|Philosophy|Perspective|Portfolio|Notice|Contact)<\/a>/g)];
